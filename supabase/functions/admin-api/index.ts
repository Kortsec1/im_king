...
Deno.serve(async req=>{
 ...
 try{
  ...
  if(postMatch&&req.method==='DELETE'){
   ...
   const {error:e}=await userClient.rpc('admin_audited_post_action',{target_post:postMatch[1],next_status:null,note:''});if(e)throw e;
   return json({ok:true});
  }
  if(postMatch&&req.method==='PATCH'){
   ...
   const {data:post,error}=await userClient.rpc('admin_audited_post_action',{target_post:postMatch[1],next_status:body.action,note});if(error)throw error;
   return json({post});
  }
